...
int xhci_alloc_virt_device(struct xhci_ctrl *ctrl, unsigned int slot_id)
{
	...
	if (ctrl->devs[slot_id]) {
		ZF_LOGE("Virt dev for slot[%d] already allocated\n", slot_id);
		return -EEXIST;
	}
	...
	if (!ctrl->devs[slot_id]) {
		ZF_LOGE("Failed to allocate virtual device\n");
		return -ENOMEM;
	}
	...
	if (!virt_dev->out_ctx) {
		ZF_LOGE("Failed to allocate out context for virt dev\n");
		return -ENOMEM;
	}
	...
	virt_dev->in_ctx = xhci_alloc_container_ctx(ctrl,
					XHCI_CTX_TYPE_INPUT);
	if (!virt_dev->in_ctx) {
		ZF_LOGE("Failed to allocate in context for virt dev\n");
		return -ENOMEM;
	}
					...
					struct xhci_hcor *hcor)
{
	...
	if (ctrl->dcbaa == NULL) {
		ZF_LOGE("unable to allocate DCBA\n");
		return -ENOMEM;
	}
				     ...
				     struct usb_device *udev, int hop_portnr)
{
	...
	case USB_SPEED_SUPER:
		...
		ZF_LOGD("Setting Packet size = 512bytes\n");
		break;
	case USB_SPEED_HIGH:
	/* USB core guesses at a 64-byte max packet first for FS devices */
	case USB_SPEED_FULL:
		ep0_ctx->ep_info2 |= cpu_to_le32(MAX_PACKET(64));
		ZF_LOGD("Setting Packet size = 64bytes\n");
		break;
	case USB_SPEED_LOW:
		ep0_ctx->ep_info2 |= cpu_to_le32(MAX_PACKET(8));
		ZF_LOGD("Setting Packet size = 8bytes\n");
		break;
	default:
		/* New speed? */
		BUG();
