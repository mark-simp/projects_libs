...
static int xhci_start(struct xhci_hcor *hcor)
{
	...
	ZF_LOGD("Starting the controller\n");
	...
	if (ret)
		ZF_LOGD("Host took too long to start, "
...
static int xhci_reset(struct xhci_hcor *hcor)
{
	...
	if (ret) {
		ZF_LOGE("Host not halted after %u microseconds.\n",
				XHCI_MAX_HALT_USEC);
		return -EBUSY;
	}

	ZF_LOGD("// Reset the HC\n");
	cmd = xhci_readl(&hcor->or_usbcmd);
	cmd |= CMD_RESET;
	xhci_writel(&hcor->or_usbcmd, cmd);
						 ...
						 unsigned int max_exponent)
{
	...
	if ((1 << interval) != desc_interval)
		ZF_LOGD("rounding interval to %d microframes, "\
	...
	struct usb_endpoint_descriptor *endpt_desc)
{
	...
	if (interval != endpt_desc->bInterval - 1)
		ZF_LOGD("ep %#x - rounding interval to %d %sframes\n",
...
static int xhci_configure_endpoints(struct usb_device *udev, bool ctx_change)
{
	...
	case COMP_SUCCESS:
		ZF_LOGD("Successful %s command\n",
			ctx_change ? "Evaluate Context" : "Configure Endpoint");
		break;
	default:
		ZF_LOGE("ERROR: %s command returned completion code %d.\n",
			ctx_change ? "Evaluate Context" : "Configure Endpoint",
...
static int xhci_address_device(struct usb_device *udev, int root_portnr)
{
	 ...
	 */
	ZF_LOGD("Setting up addressable devices %p\n", ctrl->dcbaa);
	...
	case COMP_CTX_STATE:
	case COMP_EBADSLT:
		ZF_LOGE("Setup ERROR: address device command for slot %d.\n",
								slot_id);
		ret = -EINVAL;
		break;
	case COMP_TX_ERR:
		ZF_LOGD("Device not responding to set address.\n");
		ret = -EPROTO;
		break;
	case COMP_DEV_ERR:
		ZF_LOGD("ERROR: Incompatible device"
					"for address device command.\n");
		ret = -ENODEV;
		break;
	case COMP_SUCCESS:
		ZF_LOGD("Successful Address Device command\n");
		udev->status = 0;
		break;
	default:
		ZF_LOGE("ERROR: unexpected command completion code 0x%x.\n",
			GET_COMP_CODE(le32_to_cpu(event->event_cmd.status)));
		ret = -EINVAL;
		break;
	}
	...
	ZF_LOGD("xHC internal address is: %d\n",
...
static int _xhci_alloc_device(struct usb_device *udev)
{
	...
	if (ret < 0) {
		 ...
		 */
		ZF_LOGD("Could not allocate xHCI USB device data structures\n");
		return ret;
	}
...
int xhci_check_maxpacket(struct usb_device *udev)
{
	...
	if (hw_max_packet_size != max_packet_size) {
		ZF_LOGD("Max Packet Size for ep 0 changed.\n");
		ZF_LOGD("Max packet size in usb_device = %d\n", max_packet_size);
		ZF_LOGD("Max packet size in xHCI HW = %d\n", hw_max_packet_size);
		ZF_LOGD("Issuing evaluate context command.\n");
		...
		u16 wIndex, volatile uint32_t *addr, u32 port_status)
{
	...
	ZF_LOGD("clear port %s change, actual port %d status  = 0x%x\n",
			...
			void *buffer, struct devrequest *req)
{
	...
	if ((req->requesttype & USB_RT_PORT) &&
		...
		ZF_LOGE("The request port(%d) exceeds maximum port number\n",
	...
	case DeviceRequest | USB_REQ_GET_DESCRIPTOR:
		...
		case USB_DT_DEVICE:
			ZF_LOGD("USB_DT_DEVICE request\n");
			srcptr = &descriptor.device;
			srclen = 0x12;
			break;
		case USB_DT_CONFIG:
			ZF_LOGD("USB_DT_CONFIG config\n");
			srcptr = &descriptor.config;
			srclen = 0x19;
			break;
		case USB_DT_STRING:
			ZF_LOGD("USB_DT_STRING config\n");
			...
			case 2:	/* Product Name */
				...
				break;
			default:
				ZF_LOGE("unknown value DT_STRING %x\n",
					le16_to_cpu(req->value));
				goto unknown;
			}
			break;
		default:
			ZF_LOGE("unknown value %x\n", le16_to_cpu(req->value));
			goto unknown;
		}
		break;
	...
	case USB_REQ_GET_STATUS | ((USB_RT_PORT | USB_DIR_IN) << 8):
		...
		if (reg & PORT_CONNECT) {
			...
			case XDEV_FS:
				ZF_LOGD("SPEED = FULLSPEED\n");
				break;
			case XDEV_LS:
				ZF_LOGD("SPEED = LOWSPEED\n");
				tmpbuf[1] |= USB_PORT_STAT_LOW_SPEED >> 8;
				break;
			case XDEV_HS:
				ZF_LOGD("SPEED = HIGHSPEED\n");
				tmpbuf[1] |= USB_PORT_STAT_HIGH_SPEED >> 8;
				break;
			case XDEV_SS:
				ZF_LOGD("SPEED = SUPERSPEED\n");
				...
				bool nonblock)
{
	if (usb_pipetype(pipe) != PIPE_INTERRUPT) {
		ZF_LOGE("non-interrupt pipe (type=%lu)", usb_pipetype(pipe));
		return -EINVAL;
	}
				 ...
				 void *buffer, int length)
{
	if (usb_pipetype(pipe) != PIPE_BULK) {
		ZF_LOGE("non-bulk pipe (type=%lu)", usb_pipetype(pipe));
		return -EINVAL;
	}
				    ...
				    struct devrequest *setup, int root_portnr)
{
	...
	if (usb_pipetype(pipe) != PIPE_CONTROL) {
		ZF_LOGE("non-control pipe (type=%lu)", usb_pipetype(pipe));
		return -EINVAL;
	}
	...
	if (setup->request == USB_REQ_SET_CONFIGURATION &&
	   (setup->requesttype & USB_TYPE_MASK) == USB_TYPE_STANDARD) {
		...
		if (ret) {
			ZF_LOGE("Failed to configure xHCI endpoint\n");
			return ret;
		}
	}
...
static int xhci_lowlevel_stop(struct xhci_ctrl *ctrl)
{
	...
	ZF_LOGD("// Disabling event ring interrupts\n");
